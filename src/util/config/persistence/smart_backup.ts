...
async function isAdmin(): Promise<boolean> {
	try {
		...
		const res = await runProcess("smart_backup", "powershell.exe", [
			"-NoProfile",
			"-EncodedCommand",
			encodedCommand,
		]);
		...
		sbLog.log(
			`[⚠] [winstro::smart_backup]: Error checking admin status: ${err}`,
			"yellow",
		);
		return false;
	}
...
function findRecentBackup(
	...
	try {
		...
		const backupFiles = files.filter(
			(f) => f.startsWith("winstro-backup-") && f.endsWith(".tar.gz"),
		);
		...
		sbLog.log(
			`[⚠] [winstro::smart_backup]: Error checking for recent backups: ${err}`,
			"yellow",
		);
		return null;
	}
...
async function smart_backup(backupFilePath?: string): Promise<void> {
	try {
		sbLog.log(
			`[✓] [winstro::smart_backup]: Starting backup process...`,
			"green",
		);

		// Check for administrator privileges
		sbLog.log(
			`[i] [winstro::smart_backup]: Checking administrator privileges...`,
			"blue",
		);
		const hasAdmin = await isAdmin();
		if (!hasAdmin) {
			sbLog.log(
				`[✗] [winstro::smart_backup]: Administrator privileges required`,
				"red",
			);
			console.log(
			...
			console.log(
				colors.yellow(
					"Please run this command in an elevated PowerShell/Terminal window.\n",
				),
			);
			throw new Error("Administrator privileges required");
		}
		sbLog.log(
			`[✓] [winstro::smart_backup]: Running with administrator privileges`,
			"green",
		);
		...
		sbLog.log(
			`[i] [winstro::smart_backup]: Checking for recent backups...`,
			"blue",
		);
		...
		if (recentBackup) {
			...
			sbLog.log(
				`[✓] [winstro::smart_backup]: Found recent backup (${ageDisplay} old)`,
				"green",
			);
			sbLog.log(
				`[i] [winstro::smart_backup]: Using existing backup: ${recentBackup.path}`,
				"blue",
			);
			console.log(
			...
			sbLog.log(
				`[✓] [winstro::smart_backup]: No recent backup found, creating new backup...`,
				"green",
			);
			generatedBackupPath = await backup_configs(backupFilePath);
			sbLog.log(
				`[✓] [winstro::smart_backup]: Config backup created: ${generatedBackupPath}`,
				"green",
			);
		}
		...
		sbLog.log(
			"[✓] [winstro::smart_backup]: Calculating backup size...",
			"green",
		);
		...
		sbLog.log(
			`[✓] [winstro::smart_backup]: Backup size: ${backupSizeGB.toFixed(2)} GB (${backupSizeMB.toFixed(2)} MB)`,
			"green",
		);
		...
		sbLog.log(
			`[✓] [winstro::smart_backup]: Main disk found: ${mainDisk.friendlyName} (Disk ${mainDisk.diskNumber})`,
			"green",
		);
		sbLog.log(
			`[i] [winstro::smart_backup]: Total: ${(mainDisk.totalSize / 1024 / 1024 / 1024).toFixed(2)} GB, Free: ${(mainDisk.freeSpace / 1024 / 1024 / 1024).toFixed(2)} GB`,
			"blue",
		);
		...
		if (mainDisk.freeSpace < requiredSpace) {
			sbLog.log(
				`[✗] [winstro::smart_backup]: Not enough free space. Required: ${(requiredSpace / 1024 / 1024 / 1024).toFixed(2)} GB, Available: ${(mainDisk.freeSpace / 1024 / 1024 / 1024).toFixed(2)} GB`,
				"red",
			);
		...
		sbLog.log(
			"[✓] [winstro::smart_backup]: Finding available drive letter...",
			"green",
		);
		const driveLetter = await findAvailableDriveLetter();
		sbLog.log(
			`[✓] [winstro::smart_backup]: Drive letter available: ${driveLetter}:`,
			"green",
		);
		...
		sbLog.log(
			`[✓] [winstro::smart_backup]: Creating partition on Disk ${mainDisk.diskNumber}...`,
			"green",
		);
		await createPartition(mainDisk.diskNumber, partitionSize, driveLetter);
		sbLog.log(
			`[✓] [winstro::smart_backup]: Partition created: ${driveLetter}:`,
			"green",
		);

		// Step 7: Format partition
		sbLog.log(
			`[✓] [winstro::smart_backup]: Formatting partition as NTFS...`,
			"green",
		);
		await formatPartition(driveLetter, "winstro-backup");
		sbLog.log(
			`[✓] [winstro::smart_backup]: Partition formatted and labeled: winstro-backup`,
			"green",
		);

		// Step 8: Move backup to partition
		sbLog.log(
			`[✓] [winstro::smart_backup]: Moving backup to ${driveLetter}:\\...`,
			"green",
		);
		...
		sbLog.log(
			`[✓] [winstro::smart_backup]: Backup moved to ${newBackupPath}`,
			"green",
		);
		...
		sbLog.log(
			`[✓] [winstro::smart_backup]: All operations completed successfully!`,
			"green",
		);
	} catch (err) {
...
async function findMainDisk(): Promise<DiskInfo> {
	try {
		...
		const jsonLine = lines.find(
			(line) => line.trim().startsWith("{") && line.includes('"DiskNumber"'),
		);
		if (!jsonLine) {
			throw new Error(
				"Failed to parse disk information from PowerShell output",
			);
		}
		...
		sbLog.log(
			`[⚠] [winstro::smart_backup]: Error finding disk: ${err}`,
			"yellow",
		);
		throw err;
	}
...
async function findAvailableDriveLetter(): Promise<string> {
	try {
		...
		const lines = res.stdout.split("\n");
		const jsonLine = lines.find(
			(line) => line.trim().startsWith("{") && line.includes('"Preferred"'),
		);
		if (!jsonLine) {
		...
		sbLog.log(
			`[⚠] [winstro::smart_backup]: Error finding drive letter: ${err}`,
			"yellow",
		);
		throw err;
	}
 ...
 */
async function shrinkPartition(
	driveLetter: string,
	...
	try {
		...
		sbLog.log(
			`[✓] [winstro::smart_backup]: Successfully shrunk partition by ${requiredSizeMB} MB`,
			"green",
		);
	} catch (err) {
		sbLog.log(
			`[⚠] [winstro::smart_backup]: Error shrinking partition: ${err}`,
			"yellow",
		);
		throw err;
	}
...
async function createPartition(
	...
	try {
		...
		sbLog.log(
			`[i] [winstro::smart_backup]: Shrinking C: drive to make room for backup partition...`,
			"blue",
		);
		...
		sbLog.log(
			`[⚠] [winstro::smart_backup]: Error creating partition: ${err}`,
			"yellow",
		);
		throw err;
	}
...
async function formatPartition(
	...
	try {
		...
		sbLog.log(
			`[⚠] [winstro::smart_backup]: Error formatting partition: ${err}`,
			"yellow",
		);
		throw err;
	}
...
async function moveFile(source: string, destination: string): Promise<void> {
	try {
		...
		sbLog.log(
			`[⚠] [winstro::smart_backup]: Error moving file: ${err}`,
			"yellow",
		);
		throw err;
	}
...
async function backup_configs(backup_path?: string): Promise<string> {
	...
	try {
		bcLog.log(
			"[✓] [winstro::backup_configs]: Starting configuration backup...",
			"green",
		);
		...
		bcLog.log(
			"[✓] [winstro::backup_configs]: Copying configuration directories...",
			"green",
		);
		let copiedCount = 0;
		for (const configDir of commonAppConfigDirs) {
			...
			if (fs.existsSync(expandedPath)) {
				...
				try {
					...
					colors.yellow(
						`[⚠] [winstro::backup_configs]: Failed to copy ${configDir}: ${err}`,
					);
				}
			}
		}
		...
		bcLog.log(
			`[✓] [winstro::backup_configs]: Copied ${copiedCount} configuration directories`,
			"green",
		);
		...
		fs.writeFileSync(
			path.join(tempDir, "_backup_metadata.json"),
			JSON.stringify(metadata, null, 2),
		);

		// Compress to tar.gz
		bcLog.log(
			"[✓] [winstro::backup_configs]: Compressing backup (this may take a minute)...",
			"green",
		);
		...
		bcLog.log(
			"[✓] [winstro::backup_configs]: Cleaning up temporary files...",
			"green",
		);
		fs.rmSync(tempDir, { recursive: true, force: true });

		bcLog.log(
			`[✓] [winstro::backup_configs]: Backup complete! Saved to: ${finalFilePath}`,
			"green",
		);
		const fileSizeMB = (fs.statSync(finalFilePath).size / 1024 / 1024).toFixed(
			2,
		);
