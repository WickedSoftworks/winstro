 ...
 * @param backupFilePath Optional explicit path to a backup archive
 * @returns void
 */
async function smart_restore(backupFilePath?: string): Promise<void> {
	try {
		srLog.log(
			"[✓] [winstro::smart_restore]: Checking for winstro backup partition...",
			"green",
		);

		if (backupFilePath?.trim()) {
			const explicitPath = backupFilePath.trim();
			srLog.log(
				`[✓] [winstro::smart_restore]: Using explicit backup path: ${explicitPath}`,
				"green",
			);
			await restore_configs(explicitPath);
			return;
		}
		...
		if (backupDrive) {
			srLog.log(
				`[✓] [winstro::smart_restore]: Found backup partition at ${backupDrive}`,
				"green",
			);

			// Ensure drive path has backslash
			const drivePath = backupDrive.endsWith("\\")
				? backupDrive
				: `${backupDrive}\\`;
			...
			try {
				...
				if (backupFiles.length > 0) {
					srLog.log(
						`[✓] [winstro::smart_restore]: Found ${backupFiles.length} backup file(s)`,
						"green",
					);
					...
					srLog.log(
						`[✓] [winstro::smart_restore]: Using backup: ${latestBackup}`,
						"green",
					);
					...
					srLog.log(
						"[✓] [winstro::smart_restore]: Restoration complete, cleaning up backup partition...",
						"green",
					);
					...
					srLog.log(
						"[⚠] [winstro::smart_restore]: No backup files found on partition",
						"yellow",
					);
				}
			} catch (err) {
				srLog.log(
					`[⚠] [winstro::smart_restore]: Error reading backup partition: ${err}`,
					"yellow",
				);
			}
		} else {
			srLog.log(
				'[⚠] [winstro::smart_restore]: Backup partition "winstro-backup" not found',
				"yellow",
			);
		}
		...
		srLog.log(
			"[✓] [winstro::smart_restore]: Checking default backup directory...",
			"green",
		);
		...
		if (fs.existsSync(defaultBackupDir)) {
			try {
				const backupFiles = fs
					.readdirSync(defaultBackupDir)
					.filter((file) => file.endsWith(".tar.gz"));

				if (backupFiles.length > 0) {
					srLog.log(
						`[✓] [winstro::smart_restore]: Found ${backupFiles.length} backup file(s) in default directory`,
						"green",
					);
					...
					srLog.log(
						"[⚠] [winstro::smart_restore]: No backup files found in default directory",
						"yellow",
					);
				}
			} catch (err) {
				srLog.log(
					`[⚠] [winstro::smart_restore]: Error reading default backup directory: ${err}`,
					"yellow",
				);
			}
		} else {
			srLog.log(
				"[⚠] [winstro::smart_restore]: Default backup directory does not exist",
				"yellow",
			);
		}
		...
		srLog.log(
			"[✓] [winstro::smart_restore]: Prompting for backup path...",
			"green",
		);
		...
		if (backupPath) {
			await restore_configs(backupPath);
			srLog.log(
				`[✓] [winstro::smart_restore]: restore_configs called with ${backupPath}`,
				"green",
			);
		} else {
			colors.yellow(
				"[✗] [winstro::smart_restore]: No backup path provided, aborting restore",
			);
		}
	} catch (err) {
		srLog.log(
			`[✗] [winstro::smart_restore]: Smart restore failed: ${err}`,
			"red",
		);
		throw err;
	}
...
async function findBackupPartition(): Promise<string | null> {
	try {
		...
		const res = await runProcess("smart_restore", "powershell.exe", [
			"-NoProfile",
			"-EncodedCommand",
			encodedCommand,
		]);
		...
		srLog.log(
			`[⚠] [winstro::smart_restore]: Error finding backup partition: ${err}`,
			"yellow",
		);
		return null;
	}
...
async function promptForPath(): Promise<string | null> {
	return new Promise((resolve) => {
		...
		rl.question(
			"[winstro::smart_restore]: Enter path to backup file (or press Enter to cancel): ",
			(answer) => {
				rl.close();
				...
				if (trimmed && fs.existsSync(trimmed)) {
					...
					srLog.log(
						`[⚠] [winstro::smart_restore]: File not found: ${trimmed}`,
						"yellow",
					);
					resolve(null);
				} else {
...
async function restore_configs(backupFilePath: string): Promise<void> {
	try {
		if (!fs.existsSync(backupFilePath)) {
			srLog.log(
				`[✗] [winstro::restore_configs]: Backup file not found: ${backupFilePath}`,
				"red",
			);
		...
		srLog.log(
			"[✓] [winstro::restore_configs]: Starting configuration restore...",
			"green",
		);
		...
		const files = extractedContents.filter((f) =>
			fs.statSync(path.join(tempDir, f)).isFile(),
		);
		const dirs = extractedContents.filter((f) =>
			fs.statSync(path.join(tempDir, f)).isDirectory(),
		);
		srLog.log(
			`[i] [winstro::restore_configs]: Extracted files: ${files.join(", ")}`,
			"blue",
		);
		srLog.log(
			`[i] [winstro::restore_configs]: Extracted directories: ${dirs.join(", ")}`,
			"blue",
		);
		...
		if (!fs.existsSync(metadataPath)) {
			srLog.log(
				"[⚠] [winstro::restore_configs]: Warning: Backup metadata not found",
				"yellow",
			);
			srLog.log(
				`[i] [winstro::restore_configs]: Looking in: ${metadataPath}`,
				"blue",
			);
		}
		...
		srLog.log(
			"[✓] [winstro::restore_configs]: Restoring configuration directories...",
			"green",
		);
		let restoredCount = 0;
		...
		for (const file of itemsToRestore) {
			...
			const _originalDirName = file.replace(/_/g, (match, _offset) => {
			...
			const originalPath = configDirs.find(
				(dir: string) => dir.replace(/[%\\:]/g, "_") === file,
			);
			...
			if (originalPath) {
				...
				try {
					...
					if (fs.existsSync(expandedPath)) {
						...
						srLog.log(
							`[⚠] [winstro::restore_configs]: Backed up existing config: ${originalPath}${backupSuffix}`,
							"yellow",
						);
					}
					...
					restoredCount++;
					srLog.log(
						`[✓] [winstro::restore_configs]: Restored ${originalPath}`,
						"green",
					);
				} catch (err) {
					srLog.log(
						`[⚠] [winstro::restore_configs]: Failed to restore ${originalPath}: ${err}`,
						"yellow",
					);
				}
			}
		...
		srLog.log(
			`[✓] [winstro::restore_configs]: Restore complete! Restored ${restoredCount} configuration directories`,
			"green",
		);
		if (metadata) {
			srLog.log(
				`[i] [winstro::restore_configs]: Backup was created on: ${metadata.timestamp}`,
				"blue",
			);
		}
	} catch (err) {
 ...
 * @returns void
 */
async function removeBackupPartitionAndExpand(
	driveLetter: string,
): Promise<void> {
	try {
		srLog.log(
			"[✓] [winstro::cleanup]: Removing backup partition and expanding main drive...",
			"green",
		);
		...
		const encodedGetInfo = Buffer.from(psGetPartitionInfo, "utf16le").toString(
			"base64",
		);
		const infoRes = await runProcess("smart_restore", "powershell.exe", [
			"-NoProfile",
			"-EncodedCommand",
			encodedGetInfo,
		]);

		const infoMatch = (infoRes.stdout || "").match(
			/DISK:(\d+):PARTITION:(\d+)/,
		);
		if (!infoMatch) {
			srLog.log(
				"[⚠] [winstro::cleanup]: Could not determine partition info, skipping cleanup",
				"yellow",
			);
			return;
		}
		...
		srLog.log(
			`[i] [winstro::cleanup]: Found backup partition: Disk ${diskNumber}, Partition ${partitionNumber}`,
			"blue",
		);
		...
		const cleanupRes = await runProcess("smart_restore", "powershell.exe", [
			"-NoProfile",
			"-EncodedCommand",
			encodedCleanup,
		]);
		...
		if (
			cleanupRes.code === 0 &&
			(cleanupRes.stdout || "").includes("SUCCESS")
		) {
			srLog.log(
				"[✓] [winstro::cleanup]: Backup partition removed and main drive expanded successfully!",
				"green",
			);
		} else {
			srLog.log(
				"[⚠] [winstro::cleanup]: Partition cleanup completed with warnings",
				"yellow",
			);
		}
	} catch (err) {
		srLog.log(
			`[⚠] [winstro::cleanup]: Failed to remove partition and expand drive: ${err}`,
			"yellow",
		);
		srLog.log(
			"[i] [winstro::cleanup]: You may need to manually remove the backup partition using Disk Management",
			"blue",
		);
	}
}
