...
export async function smart_write() {
    ...
    // Write to requirements.winget.ts
    const configPath = path.join(process.cwd(), 'config', 'requirements.winget.ts');
    const fileContent = `const winget_requirements: string[] = ${JSON.stringify(selectedPackages, null, 2)};\nexport default winget_requirements;\n`;
    
    try {
        fs.writeFileSync(configPath, fileContent, 'utf8');
        logger.log(`Successfully wrote ${selectedPackages.length} packages to requirements.winget.ts`, 'green');
